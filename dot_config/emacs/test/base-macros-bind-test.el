...
(ert-deftest oo--bind-metadata ()
  (lef! ((gensym (&optional prefix) (if (equal prefix "state-value") (intern prefix)
    ...
    (should (equal (oo--bind-metadata '(i "d" #'foo))
                   '((:char 105 :state state-value :keymap global-map :key "d" :def #'foo))))
    ...
    (should (equal (oo--bind-metadata '(i org-mode-map "d" #'foo))
                   '((:char 105 :state state-value :keymap org-mode-map :key "d" :def #'foo))))
    ...
    (should (equal (oo--bind-metadata '((n m v) "d" #'foo))
                   '((:state state-value :char 110 :keymap global-map :key "d" :def #'foo)
                     (:state state-value :char 109 :keymap global-map :key "d" :def #'foo)
                     (:state state-value :char 118 :keymap global-map :key "d" :def
                             #'foo))))
    ...
    (should (equal (oo--bind-metadata '(global-map (n i v) "d" #'foo))
                   '((:state state-value :char 110 :keymap global-map :key "d" :def #'foo)
                     (:state state-value :char 105 :keymap global-map :key "d" :def #'foo)
                     (:state state-value :char 118 :keymap global-map :key "d" :def #'foo))))
