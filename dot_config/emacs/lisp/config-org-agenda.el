...
Return -1 if B is more overdue than A.  Otherwise return 0."
  (flet! negative-p (apply-partially #'< 0))
  (flet! positive-p (apply-partially #'> 0))
  ...
  (cond ((or (and (not diff-a) (not diff-b))
             (and diff-a (positive-p diff-a) (not diff-b))
             (and (not diff-a) diff-b (positive-p diff-b))
             (and diff-a diff-b (positive-p diff-a) (positive-p diff-b))
             (and diff-a diff-b (= diff-a diff-b)))
         0)
        ((and diff-a (negative-p diff-a) (not diff-b))
         1)
        ((and (not diff-a) diff-b (negative-p diff-b))
         -1)
        ((and (negative-p diff-a) (positive-p diff-b))
         1)
        ((and (positive-p diff-a) (negative-p diff-b))
         -1)
        ((and (negative-p diff-a) (negative-p diff-b) (/= diff-a diff-b))
...
with no deadline."
  ...
  (set! diff-a (and da (float-time (time-subtract da now))))
  (set! diff-b (and db (float-time (time-subtract db now))))
  (cond ((and diff-a (positive-p diff-a) (not diff-b))
         1)
        ((and diff-b (positive-p diff-b) (not diff-a))
         -1)
        ((and diff-a diff-b (positive-p diff-a) (positive-p diff-b))
         (if (< diff-a diff-b) 1 (if (> diff-a diff-b) -1 0)))
        ((and diff-a (positive-p diff-a) diff-b (negative-p diff-b))
         1)
        ((and diff-b (positive-p diff-b) diff-a (negative-p diff-a))
         -1)
        (t
         0)))
