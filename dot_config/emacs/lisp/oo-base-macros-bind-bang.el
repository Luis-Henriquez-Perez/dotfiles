...
(defun! oo--build-metadata (args)
  ...
  (pcase args
    ...
    ;; (bind! insert "d" #'foo)
    ;; (bind! i "d" #'foo)
    (`(,(and (pred non-keyword-symbol-p) state) ,(and (pred notkeyp) key)
       ,(and (pred notkeyp) def) . ,plist)
     `(:state ,state :keymap global-map :key ,key :def ,def))
    ...
    (`(,(and (pred non-keyword-symbol-p) state) ,(and (pred keymap-symbol-p) keymap)
       ,(and (pred notkeyp) key) ,(and (pred notkeyp) def) . ,plist)
     `(:state ,state :keymap ,keymap :key ,key :def ,def))
    ...
    ;; (bind! (n m v) "d" #'foo)
    (`(,(and (pred symbol-list-p) states) ,(and (pred notkeyp) key)
       ,(and (pred notkeyp) def) . ,plist)
