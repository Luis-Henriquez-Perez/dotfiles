...
(defun! oo-main-modeline ()
  ...
  (set! separator-left (intern (format "powerline-%s-%s"
                                       (powerline-current-separator)
                                       (car powerline-default-separator-dir))))
  (set! separator-right (intern (format "powerline-%s-%s"
                                        (powerline-current-separator)
  ...
  (set! lhs (list (cond (buffer-read-only
                         (all-the-icons-material "lock" :face face3))
                        ((buffer-modified-p)
                         (require 'all-the-icons)
                         (concat (all-the-icons-material "save" :face face3)
                                 (powerline-raw " " face0))))
                  ;; (powerline-raw (all-the-icons-material "save" :face face3))
  ...
  ;;   (prepending! lhs (list (powerline-raw (symbol-name evil-state) face0)
  ;;                          (funcall separator-left face0 face1))))
