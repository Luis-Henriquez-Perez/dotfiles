...
;; Elpaca installs things asynchronously.

;;; advise =elpaca--make-process=
;; One idea is to add an advice around =elpaca-make-process= that would cause it
;; to actually not make a process and do the thing immediately.
(defun oo-do-it-synchronously-instead (orig-fn &rest args)
  ;; Just invoke the command that you would do right now.
  ())
