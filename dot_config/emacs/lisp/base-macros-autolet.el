      ...
      (while (not done-p)
        (pcase stack
          ...
          (`((nil ,join-fn ,(and (pred vectorp) vector)) . ,(guard t))
           (message "pred %S -> t" `((nil ,join-fn ,(and (pred vectorp) vector)) . ,(guard t)))
           (pop stack)
          ...
          (`((t ,fn ,cdr) (t ,join-fn ,car) . ,(guard t))
           (message "pred %S -> t" `((t ,fn ,cdr) (t ,join-fn ,car) . ,(guard t)))
           (setq stack (cddr stack))
           (push (list t fn (funcall join-fn car cdr)) stack)
           (message "stack -> %s" stack))
          (`((t ,_ ,_) (nil ,_ ,_) . ,(guard t))
           (message "pred %S -> t" `((t ,_ ,_) (nil ,_ ,_) . ,(guard t)))
           (push (pop (cdr stack)) stack)
           (message "stack -> %s" stack))
          (`((nil ,_ (,(pred quote-symbol-p) . ,_)) . (guard t))
           (message "pred %S -> t" `((nil ,_ (,(pred quote-symbol-p) . ,_)) . (guard t)))
           (setf (caar stack) t)
           (message "stack -> %s" stack))
          (`((nil ,_ (,(and name (pred ing-symbol-p)) ,symbol . ,(guard t))) . ,(guard t))
           (message "pred %S -> t" `((nil ,_ (,(and name (pred ing-symbol-p)) ,symbol . ,(guard t))) . ,(guard t)))
           (cl-case name
             ((maxing! maximizing!)
          ...
          (`((nil ,fn (,(and loop (pred loop-symbol-p)) ,pred . ,(and body (guard t)))) . ,(guard t))
           (message "pred %S -> t" `((nil ,fn (,(and loop (pred loop-symbol-p)) ,pred . ,(and body (guard t)))) . ,(guard t)))
           (setq stack (cdr stack))
           (push (list nil fn body) stack)
           (push (list nil (loop-join-fn loop pred) nil) stack)
           (message "stack -> %s" stack))
          (`((nil ,_ (set! ,pattern ,_ . ,(guard t))) . ,(guard t))
           (message "pred %S -> t" `((nil ,_ (set! ,pattern ,_ . ,(guard t))) . ,(guard t)))
           (if (symbolp pattern)
          ...
          (`((nil ,join-fn ((,(and letsym (pred letbind-symbol-p)) . ,args) . ,(and rest (guard t)))) . ,(guard t))
           (message "pred %S -> t" `((nil ,join-fn ((,(and letsym (pred letbind-symbol-p)) . ,args) . ,(and rest (guard t)))) . ,(guard t)))
           (pop stack)
