...
- *consistent and simple*
  The way that my configuration is loaded is designed to be as simple and
  consistent as possible.  No modules.
* Features
:PROPERTIES:
...
:END:
The way I choose to manage my files reflects my point on "consistent and
simple".  A simple.
...
- init files - These are files that can be loaded immediately.  Their objective
  ...
  that means adding hooks, setting bindings and autoloading functions.  These
  files should be fast to load because they (typically) do not load packages,
  instead they simple set up a package to be loaded.
- config files - These are files that are loaded after some feature is loaded.
  ...
  themselves may contain some computation that should only needs be done when a
  particular feature is loaded.
Organizing files in this way makes it dead simple to add a new feature.
* why the name =OO=?
:PROPERTIES:
