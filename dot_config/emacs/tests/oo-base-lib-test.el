...
(ert-deftest let! ()
  (should-not (mapcan #'oo--let-bind '(((foo) '(1 2 3 4)))))
  ;; (should-not (mapcar #'oo--let-bind '((#'foo (lambda ())))))
  ;; (should (= 3 (lef! ((foo (lambda (a b) (+ a b)))) (foo 1 2))))
  (should-not (macroexpand-1 '(let! (((foo) '(1 2 3 4))) 1)))
  ;; (should (= 1 (let! (((foo) '(1 2 3 4))) 1)))
  (should-not (macroexpand-1 '(let! ((#'foo (lambda (a b) (+ a b)))) (foo 1 2))))
